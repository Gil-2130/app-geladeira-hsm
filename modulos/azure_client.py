...
def atualizar_historico_bi(df_novo_dia):
    """
    O Livro-Razão do BI: Lê o histórico CSV do Azure, 
    anexa os resultados da campanha de hoje e sobrescreve.
    """
    container = st.secrets["container_name"]
    blob_name = 'Historico_BI_Geladeira.csv'
    
    blob_service_client = obter_cliente_blob()
    blob_client = blob_service_client.get_blob_client(container=container, blob=blob_name)
    
    # 1. Tenta baixar o Livro-Razão antigo
    try:
        stream_download = blob_client.download_blob()
        df_historico_antigo = pd.read_csv(io.BytesIO(stream_download.readall()), sep=';', dtype=str)
    except Exception:
        # Se o arquivo não existir no Azure (primeiro dia de uso), cria uma base vazia
        df_historico_antigo = pd.DataFrame()
        
    # 2. Anexa (Append) a campanha de hoje ao histórico geral e blinda contra duplicidades
    colunas_chave = ['WhatsAppdoContato', 'Carteira', 'Data_Filtragem']
    
    if not df_historico_antigo.empty:
        df_consolidado = pd.concat([df_historico_antigo, df_novo_dia], ignore_index=True)
        # Guilhotina: Garante que o mesmo telefone não passe duas vezes na mesma campanha no mesmo dia
        df_consolidado = df_consolidado.drop_duplicates(subset=colunas_chave, keep='last')
    else:
        df_consolidado = df_novo_dia.drop_duplicates(subset=colunas_chave, keep='last')
        
    # 3. Salva de volta no Azure em formato CSV (Mais leve para o Power BI)
    buffer = io.BytesIO()
    df_consolidado.to_csv(buffer, sep=';', index=False, encoding='utf-8-sig')
    blob_client.upload_blob(buffer.getvalue(), overwrite=True)
